    ...
    return(
        <div className={`flex justify-center mt-10 font-display mb-10 ${props.isFocus ? 'text-TextLight' : 'text-BreakText'}`}>           
            <div className="flex flex-col w-100">
                <ul className="">
                    {task.length === 0 ? '' : 
                    <p className={`text-xl font-semibold ${props.isFocus ? 'dark:text-FocusTextDark' : 'dark:text-BreakTextDark'} 
                    border-b py-2 -mt-3 mb-6 max-sm:w-75 mx-auto`}>Tasks</p>
                    }
                    {task.map((list) => {
                    const handleRemove = () => {
                    ...
                    relative text-xl font-medium mt-2 p-2 py-3 ${list.isSetting ? 'pl-5' : 'pl-12'} rounded-md max-w-md max-sm:w-75 mx-auto`}>
                    {list.isSetting ? 
                    <form>
                        ...
                        <textarea name="description" id="description" defaultValue={list.desc} onChange={(e) => {setDescription(e.target.value)}} placeholder={'Add some notes...'} cols={window.innerWidth <= 600 ? 26 : 37}
                ...
                {addTask ? 
                <form className={`${props.isFocus ? 'bg-Focus-component/30 dark:text-FocusTextDark' :
                    'bg-Break-component/30 dark:text-BreakTextDark'} text-xl font-medium mt-2 py-3 p-2 pl-5 rounded-md max-sm:w-75 mx-auto max-w-md`}>
                    ...
                    <p className="italic text-lg mt-2">Description</p>
                    <textarea name="description" id="description" placeholder={'Add some notes...'} onChange={(e) => {setDescription(e.target.value)}} cols={window.innerWidth <= 600 ? 26 : 37}
                    className="text-base bg-FocusLight dark:text-black rounded p-2 focus:outline-none"></textarea>
                    <div className="flex justify-between pr-3">
                        <label htmlFor="pomodoro" className="text-lg">Pomodoro</label>
                        <input type="number" name="pomodoro" id="pomodoro" max={4} min={1} defaultValue={1}
                ...
                className={`${props.isFocus ? 'dark:text-FocusTextDark bg-Focus-component/30 border-Focus-component hover:bg-Focus-component/80' : 
                'dark:text-BreakTextDark border-Break-component bg-Break-component/30 hover:bg-Break-component/60'}
                transition ease-in-out duration-150 p-2 mt-5 w-100 mx-auto max-sm:w-75 px-3 border border-dashed rounded`}>
